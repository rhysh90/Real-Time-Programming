...
package body Simtrack2.Display is
   Version_String : constant String := "Train Track Simulation 2.0";
  ...
  procedure Show_Dac(Dacno : in Train_Id;
                       Voltage : in Float) is
      Temp : String(1..6);
      X : Integer :=  X_Size - 200 + 2*X_Char;  -- was 90
      Y : Integer := Y_Zero + 60 + 20*Integer(Dacno);
      W : Integer := 6*X_Char;  -- approx 54
   begin
      Temp := "      ";
      Int_Io.Put(Temp, abs Integer(Voltage*1000.0));
...
--      & scaled_height'img & scaled_radius'img & yc'img & cab_y(4)'img);
      ...
      Ypos := Y_Zero + 60;
      Display_Text(Xpos - 5, Ypos, "DAC    mV", Fg_Color);
